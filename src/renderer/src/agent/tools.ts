...
export async function executeToolCall(
    ...
    // Route to plugin-mode handlers when applicable
    if (context?.syncMode === 'plugin') {
        return executePluginToolCall(toolName, args, context.projectId)
    }

    // Default: filesystem mode
    return executeFilesystemToolCall(toolName, args, projectPath)
}

/**
 * Execute tool calls using the plugin server (virtual file system + command queue).
 */
async function executePluginToolCall(
    toolName: string,
    args: Record<string, any>,
    projectId: string
): Promise<ToolResult> {
    try {
        switch (toolName) {
            case 'read_file': {
                const result = await window.api.plugin.readFile(projectId, args.path)
                if (result.success) {
            ...
            case 'write_file': {
                const result = await window.api.plugin.writeFile(projectId, args.path, args.content)
                if (result.success) {
            ...
            case 'edit_file': {
                // Read from virtual file store
                const readResult = await window.api.plugin.readFile(projectId, args.path)
                ...
                const writeResult = await window.api.plugin.writeFile(projectId, args.path, newContent)
            ...
            case 'delete_file': {
                const result = await window.api.plugin.deleteFile(projectId, args.path)
                if (result.success) {
            ...
            case 'list_directory': {
                const files = await window.api.plugin.getFiles(projectId)
                const targetPath = args.path === '.' ? '' : args.path

                // Filter files based on requested directory
                const filtered = files.filter(f => {
                    if (!targetPath) return true
                    return f.path.startsWith(targetPath + '/') || f.path === targetPath
                })

                if (!args.recursive) {
                    // Only show direct children
                    const depth = targetPath ? targetPath.split('/').length : 0
                    const directChildren = filtered.filter(f => {
                        const fileParts = f.path.split('/')
                        return fileParts.length === depth + 1
                    })
                    const formatted = directChildren.map(f =>
            ...
            case 'search_files': {
                ...
                for (const file of files) {
                    ...
                    if (args.extensions) {
                        ...
                        if (!ext || !args.extensions.includes(ext)) continue
                    }
                    ...
                    // Search content
                    const readResult = await window.api.plugin.readFile(projectId, file.path)
                    if (readResult.success && readResult.content) {
                        const lines = readResult.content.split('\n')
                        for (let i = 0; i < lines.length; i++) {
                            if (lines[i].toLowerCase().includes(queryLower)) {
                                results.push(`📄 ${file.path}:${i + 1} - ${lines[i].trim().substring(0, 100)}`)
                            }
                        }
                    }
                }
...
/**
 * Execute tool calls using the filesystem (original behavior).
 */
async function executeFilesystemToolCall(
    toolName: string,
    args: Record<string, any>,
    projectPath: string
): Promise<ToolResult> {
